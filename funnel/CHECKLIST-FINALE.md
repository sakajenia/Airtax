...
| **6** | **Template email «Report Calcolatore Airbnb»**: importa `funnel/report-email.html` (Import HTML / Code). Merge tag già con chiavi canoniche + `{{contact.first_name \| Host}}` | **H-bis** | `app.gohighlevel.com/location/E1HO8PRyWf2yGaTFLuLC/emails/templates` |
...
| **8** | **Dominio `tool.affittibreviaroma.com`**: DNS CNAME (valore fornito da GHL, Cloudflare in "DNS only") → Settings/Domains → Add → Verify → assegna al funnel, home = `/` | **A** | `app.gohighlevel.com/location/E1HO8PRyWf2yGaTFLuLC/settings/domain` |
